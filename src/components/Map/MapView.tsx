...
export default function MapView({ groupMembers = [], currentUserId, groupId, onPanelChange, followUserId, onFollowChange, requestNavTo, onNavRequested, onNavigationChange, searchCategory = null, onSOS, onModeSelectorChange, workMode = false, onReport, onCategorySearching, controlsTopOffset = 12, externalReportOpen = false }: Props) {
  ...
  // Session start — only show incidents created after the app opened
  const sessionStartRef = useRef(new Date())

  // Keep incident subscription active always — including during navigation
  useEffect(() => {
    setIncidents([])
  ...
  const interactiveLayerIds = useMemo(() => {
    ...
    // Zone fills are NOT interactive — only their center pins are (via React Markers)
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              {[
                ...
                { count: filteredIncidents.length, label: `Reports ${timeFilter}h`, color: '#4f8ef7', bg: 'rgba(45,111,232,0.08)', border: 'rgba(45,111,232,0.2)' },
